...
/**
 * Queues are assigned to threads round-robin. Queue i is on thread
 * (i % num_threads). Each thread has its array of queues and manages a skiplist
 * of active queues (with avail > 0) by their next time they get to dequeue.
 * Queues without a limit are tracked in a separate list.
 */

#define FLAG_INSKIPLIST 1
#define FLAG_INNOLIMITL 2

/** Number of per-thread operations queue */
#define OPS_NUM 1024
...
struct queue {
  ...
  uint32_t next_idxs[SKIPLIST_LEVELS];
  /** Time stamp */
  uint32_t next_ts;
  /** Assigned Rate */
  uint32_t rate;
  ...
  uint16_t max_chunk;
  /** Opaque value returned when queue fires */
  uint8_t opaque;
...
/**
 * Per-thread state for queue manager.
 *
 * Split up into parts that are read-only, only modified by the owner, and
 * modified by other threads to avoid false sharing.
 */
struct qman_thread {
  /************************************/
  /* read-only */
  ...
  uint32_t head_idx[SKIPLIST_LEVELS] __attribute__((aligned(64)));
...
static struct qman_thread **qthreads;
...
static inline void set_impl(struct qman_thread *t, uint32_t id, uint32_t rate,
    uint32_t avail, uint16_t max_chunk, uint8_t opaque, uint8_t flags);
...
static inline unsigned poll_nolimit(struct qman_thread *t, uint32_t cur_ts,
    unsigned num, unsigned *q_ids, uint16_t *q_bytes, uint8_t *q_opaques);
...
int qman_init(unsigned nthreads)
{
  num_threads = nthreads;

  if ((qthreads = rte_zmalloc("qman qthreads array",
          sizeof(*qthreads) * nthreads, 0)) == NULL)
  {
    fprintf(stderr, "qman_init: malloc qthreads array failed\n");
    return -1;
  }
...
struct qman_thread *qman_thread_init(uint16_t id)
{
  ...
  if ((t = rte_zmalloc("qman thread", sizeof(*t), 0)) == NULL) {
    fprintf(stderr, "qman_thread_init: thread malloc failed\n");
    return NULL;
  }
  ...
  utils_rng_init(&t->rng, RNG_SEED * id + id);
...
int qman_poll(struct qman_thread *t, unsigned num, unsigned *q_ids,
    uint16_t *q_bytes, uint8_t *q_opaques)
{
  unsigned x, y;
  uint32_t ts = timestamp();
  ...
  if (t->nolimit_first) {
    x = poll_nolimit(t, ts, num, q_ids, q_bytes, q_opaques);
    y = poll_skiplist(t, ts, num - x, q_ids + x, q_bytes + x, q_opaques + x);
  } else {
    x = poll_skiplist(t, ts, num, q_ids, q_bytes, q_opaques);
    y = poll_nolimit(t, ts, num - x, q_ids + x, q_bytes + x, q_opaques + x);
  }
    ...
    uint16_t max_chunk, uint8_t opaque, uint8_t flags)
{
  ...
  struct flexnic_trace_entry_qman_set evt = {
      ...
      .opaque = opaque, .flags = flags,
    };
  trace_event(FLEXNIC_TRACE_EV_QMSET, sizeof(evt), &evt);
#endif

  dprintf("qman_set: id=%u rate=%u avail=%u max_chunk=%u opaque=%u, qidx=%u tid=%u\n",
      id, rate, avail, max_chunk, opaque, qidx, tid);
  ...
  set_impl(t, id, rate, avail, max_chunk, opaque, flags);
    ...
    uint32_t avail, uint16_t max_chunk, uint8_t opaque, uint8_t flags)
{
  ...
  dprintf("set_impl: t=%p q=%p idx=%u avail=%u rate=%u qflags=%x flags=%x opaque=%u\n", t, q, idx, q->avail, q->rate, q->flags, flags, opaque);
    ...
    unsigned num, unsigned *q_ids, uint16_t *q_bytes, uint8_t *q_opaques)
{
  ...
  for (cnt = 0; cnt < num && t->nolimit_head_idx != IDXLIST_INVAL;) {
    ...
    if (q->avail > 0) {
      queue_fire(t, q, idx, q_ids + cnt, q_bytes + cnt, q_opaques + cnt);
      cnt++;
    }
  }
    ...
    struct queue *q, uint32_t q_idx)
{
  uint8_t level;
  int8_t l;
  uint32_t preds[SKIPLIST_LEVELS];
    ...
    unsigned num, unsigned *q_ids, uint16_t *q_bytes, uint8_t *q_opaques)
{
  ...
  for (cnt = 0; cnt < num;) {
    ...
    for (l = 0; l < SKIPLIST_LEVELS && t->head_idx[l] == idx; l++) {
...
static inline uint8_t queue_level(struct qman_thread *t)
{
  ...
  return (x < SKIPLIST_LEVELS ? x : SKIPLIST_LEVELS - 1);
}
    ...
    uint8_t *q_opaque)
{
  ...
  dprintf("queue_fire: t=%p q=%p idx=%u gidx=%u bytes=%u avail=%u rate=%u opaque=%u\n", t, q, idx, idx, bytes, q->avail, q->rate, q->opaque);
  if (q->rate > 0) {
  ...
  *q_opaque = q->opaque;
  ...
  struct flexnic_trace_entry_qman_event evt = {
      .id = *q_id, .bytes = bytes, .opaque = q->opaque,
    };
