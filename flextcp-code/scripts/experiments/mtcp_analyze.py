...
def parse_experiment(scn):
    m = re.match(r'conns_stack([^_]+)_fc(\d+)_co(\d+)_cc(\d+)', scn)
    if not m:
        return None
...
def parse_client(f):
    ...
    for l in f:
        ...
        conn_m = re.match(r'O ([^:]+):\s+t\[(\d+)\]\.conns\[(\d+)\]: +pend=(\d+) +rx_r=\d+ +tx_r=\d+ +cnt=(\d+) +fd=\d+', l)
        ...
        tp_m = re.match(r'O ([^:]+): TP: total=([0-9\.]+) mbps  50p=(-?[0-9]+) us  90p=(-?[0-9]+) us  95p=(-?[0-9]+) us  99p=(-?[0-9]+) us  99\.9p=(-?[0-9]+) us  99.99p=(-?[0-9]+) us.*', l)
        if tp_m is not None:
            meas = {
                'tput': float(tp_m.group(2)),
                '50p': negnone(tp_m.group(3)),
                '90p': negnone(tp_m.group(4)),
                '95p': negnone(tp_m.group(5)),
                '99p': negnone(tp_m.group(6)),
                '99.9p': negnone(tp_m.group(7)),
                '99.99p': negnone(tp_m.group(8)),
                'conns': {},
                'cores': {}
                }
