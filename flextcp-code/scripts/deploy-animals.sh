...
for m in $machines; do
  run_tagged $m rsync -av $RSYNC_EXCL . $m:/scratch/antoinek/flextcp-code &
done
wait
...
for m in $machines; do
  ...
  run_tagged_on $m "cd /scratch/antoinek/flextcp-code && $cleancmd && make -j12 all &&  make flexkvs-linux && cd mtcp && make clean && make -j8" &
done
wait
