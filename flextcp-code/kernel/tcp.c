...
struct connection *tcp_conns = NULL;
    ...
    uint16_t flow_group)
{
  ...
  if ((c = conn_lookup(p)) != NULL) {
    ...
    fprintf(stderr, "tcp_packet: unknown connection\n");
    send_reset(p, &opts);
  }
}
...
static inline uint16_t port_alloc(void)
{
  ...
  while (wrapped <= 1) {
    if ((ports[p] & PORT_TYPE_MASK) == PORT_TYPE_UNUSED) {
      port_eph_hint = (p + 1 < PORT_FIRST_EPH ? PORT_FIRST_EPH : p + 1);
      return p;
    }

    p = (p + 1 < PORT_FIRST_EPH ? PORT_FIRST_EPH : p + 1);
  }
...
static inline struct connection *conn_alloc(void)
{
  ...
  conn->rx_buf = (uint8_t *) packetmem + off_rx;
  conn->rx_len = config.tcp_rxbuf_len;
  conn->tx_buf = (uint8_t *) packetmem + off_tx;
...
void conn_failed(struct connection *c, int status)
{
  ...
  c_p = NULL;
  for (c_i = tcp_conns; c_i != NULL; c_i = c_i->hash_next) {
    if (c_i == c) {
      if (c_p == NULL) {
        tcp_conns = c->hash_next;
      } else {
        c_p->hash_next = c->hash_next;
      }
  ...
  fprintf(stderr, "conn_failed called on connection not in list\n");
  abort();
}
