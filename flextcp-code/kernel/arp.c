...
static inline int response_tx_tap(const void *src_mac, const void *dst_mac, uint32_t src_ip, uint32_t dst_ip);
...
int arp_init(void)
{
  ...
  lb->status = 0;
  lb->ip = config.ip;
  memcpy(lb->mac, &flexnic_info->mac_addr, ETH_ADDR_LEN);
  lb->compl = NULL;
  lb->next = NULL;
  lb->prev = NULL;
  arp_table = lb;

  printf("host ip: %x MAC: %lu\n", config.ip, flexnic_info->mac_addr);
...
void arp_packet_tap(const void *pkt, uint16_t len)
{
  ...
  if (op == ARP_OPER_REQUEST) {
    ...
    /* send response */
    if (response_tx_tap(&arp->tha, &arp->sha, f_beui32(arp->tpa), f_beui32(arp->spa)) != 0) {
    ...
    fprintf(stderr, "tap doesn't know how to handle this\n");
    return;
    /*fprintf(stderr, "tap arp? %hhX %hhX %hhX %hhX %hhX %hhX\n", arp->tha.addr[0], arp->tha.addr[1],
			arp->tha.addr[2], arp->tha.addr[3], arp->tha.addr[4], arp->tha.addr[5]);
    if (memcmp(&flexnic_info->mac_addr, &arp->tha, ETH_ADDR_LEN) != 0){
	tap_write((uint8_t*) pkt, len);
	return;
    }*/
    /* handle ARP response */
    if ((ae = ae_lookup(f_beui32(arp->spa))) == NULL) {
...
static inline int response_tx_tap(const void *src_mac, const void *dst_mac, uint32_t src_ip, uint32_t dst_ip)
{
  ...
  /* fill in response */
  memcpy(&parp_out.eth.src, src_mac, ETH_ADDR_LEN);
  memcpy(&parp_out.arp.sha, src_mac, ETH_ADDR_LEN);
  memcpy(&parp_out.eth.dest, dst_mac, ETH_ADDR_LEN);
  memcpy(&parp_out.arp.tha, dst_mac, ETH_ADDR_LEN);
  parp_out.arp.spa = t_beui32(src_ip);
  ...
  tap_write((uint8_t*) &parp_out, sizeof(parp_out));
...
static inline int response_tx(const void *dst_mac, uint32_t dst_ip)
{
  ...
  /* fill in response */
  memcpy(&parp_out->eth.src, &flexnic_info->mac_addr, ETH_ADDR_LEN);
  memcpy(&parp_out->arp.sha, &flexnic_info->mac_addr, ETH_ADDR_LEN);
  memcpy(&parp_out->eth.dest, dst_mac, ETH_ADDR_LEN);
  memcpy(&parp_out->arp.tha, dst_mac, ETH_ADDR_LEN);
