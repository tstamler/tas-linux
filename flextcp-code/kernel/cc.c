...
unsigned cc_poll(uint32_t cur_ts)
{
  ...
  if (diff_ts < config.cc_control_granularity)
    return 0;
  ...
  for (c = tcp_conns; c != NULL; c = c->hash_next) {
    ...
    kstats.ecn_marked += stats.c_ecn;
    kstats.acks += stats.c_acks;
    ...
    struct nicif_connection_stats *stats, uint32_t diff_ts, uint32_t cur_ts)
{
  ...
  /* Slow start */
  if (cc->slowstart) {
    if (stats->c_drops == 0 && stats->c_ecn == 0 && c->cc_rexmits == 0) {
  ...
  if (!cc->slowstart) {
    ...
    if (stats->c_drops > 0 || c->cc_rexmits > 0) {
      ...
      /* update ECN rate */
      if (stats->c_acks > 0) {
        stats->c_ecn = (stats->c_ecn <= stats->c_acks ? stats->c_ecn :
            stats->c_acks);
        ecn_rate = (((uint64_t) stats->c_ecn) * UINT32_MAX) / stats->c_acks;
    ...
    struct nicif_connection_stats *stats, uint32_t diff_ts, uint32_t cur_ts)
{
  struct connection_cc_dctcp_rate *cc = &c->cc.dctcp_rate;
  uint64_t ecn_rate;
  uint32_t act_rate, rate = c->cc_rate, rtt = stats->rtt, c_ecn, c_acks, c_ackb,
           c_drops;
  ...
  c_ecn = cc->unproc_ecn + stats->c_ecn;
  ...
  /* Slow start */
  if (cc->slowstart) {
    if (c_drops == 0 && c_ecn == 0 && c->cc_rexmits == 0) {
      /* double rate*/
      if (rate * 2 >= rate)
        rate *= 2;
      else
  ...
  if (!cc->slowstart) {
    ...
    if (c_drops > 0 || c->cc_rexmits > 0) {
      ...
      /* update ECN rate */
      if (c_acks > 0) {
        c_ecn = (c_ecn <= c_acks ? c_ecn : c_acks);
        ecn_rate = (((uint64_t) c_ecn) * UINT32_MAX) / c_acks;
