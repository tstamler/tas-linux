...
int appif_init(void)
{
  ...
  for (i = flexnic_info->db_num - 1; i > 0; i--) {
...
unsigned appif_poll(void)
{
  ...
  ssize_t ret;
  uint16_t i;
  uint64_t rxq_offs[flexnic_info->cores_num], txq_offs[flexnic_info->cores_num];
  uint64_t cnt = 1;
  unsigned n = 0;
...
static void uxsocket_accept(void)
{
  ...
  struct iovec iov = {
    .iov_base = &flexnic_info->cores_num,
  ...
  *pfd = kernel_notifyfd;
  ...
  off = 0;
  for (; off < flexnic_info->cores_num;) {
    iov.iov_base = &b;
    iov.iov_len = 1;
    ...
    n = (flexnic_info->cores_num - off >= 4 ? 4 : flexnic_info->cores_num - off);
  ...
  sz = sizeof(*app->resp) +
    flexnic_info->cores_num * sizeof(app->resp->flexnic_qs[0]);
  app->resp_sz = sz;
...
static void uxsocket_receive(struct application *app)
{
  ...
  ctx_sz = sizeof(*ctx) + flexnic_info->cores_num * sizeof(ctx->handles[0]);
  ...
  for (i = 0; i < flexnic_info->cores_num; i++) {
    ...
    memset((uint8_t *) packetmem + off_rxq, 0, app->req.rxq_len);
    memset((uint8_t *) packetmem + off_txq, 0, app->req.txq_len);
  ...
  app->resp->flexnic_qs_num = flexnic_info->cores_num;
  app->resp->status = 0;
