...
static struct qman_queue *qman_alloc_ctx(void);
static struct qman_queue *qman_alloc_flow(uint32_t core);

void *packetmem = NULL;
struct flextcp_pl_mem *intmem = NULL;
struct flexnic_info *flexnic_info = NULL;

static uint32_t fn_cores;
static uint32_t flow_id = 0;
struct qman_queue **qman_queues;
static uint32_t qman_ctx_core;
...
{
  int ret;
  struct qman_queue *qq;
  uint32_t i, c;

  rte_hash_crc_init_alg();

  /* try to connect to flexnic driver */
  do {
    ret = flexnic_driver_connect(&flexnic_info, &packetmem);
  } while (ret > 0);

  if (ret < 0) {
    fprintf(stderr, "nicif_init: flexnic_driver_connect failed\n");
    return -1;
  }

  /* connect to internal memory */
  if (flexnic_driver_internal((void **) &intmem) != 0) {
    fprintf(stderr, "nicif_init: flexnic_driver_internal failed\n");
    return -1;
  }

  fn_cores = flexnic_info->cores_num;
  ...
  /* prepare qman queue allocator */
  if ((qman_queues = calloc(fn_cores, sizeof(*qman_queues))) == NULL) {
    fprintf(stderr, "nicif_init: queue manager queue array alloc failed\n");
    return -1;
  }
  for (i = flexnic_info->qmq_num - 1; i > 0; i--) {
    if ((qq = malloc(sizeof(*qq))) == NULL) {
      fprintf(stderr, "nicif_init: qq malloc failed\n");
      return -1;
    }

    c = i % fn_cores;
    qq->id = i;
    qq->next = qman_queues[c];
    qman_queues[c] = qq;
  }
  qman_ctx_core = 1 % fn_cores; /* kernel context is on core 0 */
    ...
    uint32_t rxq_len, uint64_t *txq_base, uint32_t txq_len, int evfd)
{
  ...
  struct flextcp_pl_appst *ast = &intmem->appst[appid];
  struct qman_queue *qq;
  uint16_t i;
  ...
  if ((qq = qman_alloc_ctx()) == NULL) {
    fprintf(stderr, "nicif_appctx_add: qman queue alloc failed\n");
    return -1;
  }
  intmem->qmant[qq->id].id = db | FLEXNIC_PL_QMANTE_TYPEACTX;

  for (i = 0; i < flexnic_info->cores_num; i++) {
    actx = &intmem->appctx[i][db];
    actx->qman_qid = qq->id;
    ...
    uint32_t *pf_id)
{
  ...
  struct flextcp_pl_flowhte *hte = intmem->flowht;
  struct qman_queue *qq;
  ...
  /* allocate queue manager queue */
  if ((qq = qman_alloc_flow(fn_core)) == NULL) {
    fprintf(stderr, "nicif_connection_add: qman queue alloc failed\n");
    return -1;
  }
  ...
  intmem->qmant[qq->id].id = f_id | FLEXNIC_PL_QMANTE_TYPEFLOW;
...
{
  intmem->flowst[f_id].db_id = dst_db;
  return 0;
}
    ...
    struct nicif_connection_stats *p_stats)
{
  ...
  fs = &intmem->flowst[f_id];
  p_stats->c_drops = __sync_fetch_and_and(&fs->cnt_tx_drops, 0);
  p_stats->c_acks = __sync_fetch_and_and(&fs->cnt_rx_acks, 0);
  p_stats->c_ackb = __sync_fetch_and_and(&fs->cnt_rx_ack_bytes, 0);
  p_stats->c_ecn = __sync_fetch_and_and(&fs->cnt_rx_ecn, 0);
...
int nicif_connection_retransmit(uint32_t f_id, uint16_t flow_group)
{
  ...
  uint32_t tail;
  uint16_t core = intmem->flow_group_steering[flow_group];
  ...
  util_flexnic_kick(&intmem->kctx[core]);
...
void nicif_tx_send(uint32_t opaque)
{
  ...
  util_flexnic_kick(&intmem->kctx[0]);
}
...
static int adminq_init_core(uint16_t core)
{
  ...
  rxq_base[core] = (volatile struct flextcp_pl_krx *)
      ((uint8_t *) packetmem + off_rx);
  txq_base[core] = (volatile struct flextcp_pl_ktx *)
      ((uint8_t *) packetmem + off_tx);
  ...
  for (i = 0; i < rxq_len; i++) {
    ...
    rxq_bufs[core][i].buf = (uint8_t *) packetmem + off_bufs;
    rxq_base[core][i].addr = off_bufs;
    off_bufs += PKTBUF_SIZE;
  }
  for (i = 0; i < txq_len; i++) {
    txq_bufs[core][i].addr = off_bufs;
    txq_bufs[core][i].buf = (uint8_t *) packetmem + off_bufs;
    off_bufs += PKTBUF_SIZE;
  }

  intmem->kctx[core].rx_base = off_rx;
  intmem->kctx[core].tx_base = off_tx;
  MEM_BARRIER();
  intmem->kctx[core].tx_len = sz_tx;
  intmem->kctx[core].rx_len = sz_rx;
  return 0;
}
...
static struct qman_queue *qman_alloc_ctx(void)
{
  ...
  do {
    c = qman_ctx_core;
    qq = qman_queues[c];
    qman_ctx_core = (c + 1) % fn_cores;

    if (qq != NULL) {
      qman_queues[c] = qq->next;
      return qq;
    }
  } while (++i < fn_cores);
...
static struct qman_queue *qman_alloc_flow(uint32_t core)
{
  ...
  assert(core < fn_cores);
  qq = qman_queues[core];
  if (qq == NULL) {
    return NULL;
  }

  qman_queues[core] = qq->next;
  return qq;
}
