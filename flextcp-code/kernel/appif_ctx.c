...
void appif_conn_opened(struct connection *c, int status)
{
  ...
  if (status == 0) {
    kout->data.conn_opened.rx_off = c->rx_buf - (uint8_t *) packetmem;
    kout->data.conn_opened.tx_off = c->tx_buf - (uint8_t *) packetmem;
...
void appif_accept_conn(struct connection *c, int status)
{
  ...
  if (status == 0) {
    kout->data.accept_connection.rx_off = c->rx_buf - (uint8_t *) packetmem;
    kout->data.accept_connection.tx_off = c->tx_buf - (uint8_t *) packetmem;
