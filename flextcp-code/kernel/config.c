...
static void config_defaults(struct configuration *c);
static void print_usage(struct configuration *c);
...
static void config_defaults(struct configuration *c)
{
  ...
  c->arp_to = 500;
  c->arp_to_max = 1000000;
  c->tcp_rtt_init = 50;
  c->tcp_link_bw = 10;
  c->tcp_rxbuf_len = 1024 * 1024;
  c->tcp_txbuf_len = 1024 * 1024;
...
static void print_usage(struct configuration *c)
{
  fprintf(stderr, "Usage: kernel [OPTION]... --ip-addr=IP[/PREFIXLEN]\n"
      "\n"
      "Kernel queues:\n"
